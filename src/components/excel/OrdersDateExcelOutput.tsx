...
interface OrdersDateExcelOutputProps {
    startDate: string,
    endDate: string,
}

export const OrdersDateExcelOutput: React.FC<OrdersDateExcelOutputProps> = ({startDate, endDate}) => {
    const handleExcel = async (startDate: string, endDate: string) => {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/api/v1/orders/excel?startDate=${startDate}&endDate=${endDate}`, {
                headers,
                responseType: 'blob',
            });
