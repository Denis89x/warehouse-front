...
import './OrderExcel.css'

interface DeliveryNoteExcelProps {
    startDate: string,
    endDate: string,
}

export const DeliveryNoteExcel: React.FC<DeliveryNoteExcelProps> = ({startDate, endDate}) => {
    const handleExcel = async (startDate: string, endDate: string) => {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/api/v1/orders/excel-delivery-note?startDate=${startDate}&endDate=${endDate}`, {
                headers,
                responseType: 'blob',
            });
            ...
            if (response.status === 200) {
                ...
                link.href = url;
                link.setAttribute('download', 'DeliveryNote.xlsx');
