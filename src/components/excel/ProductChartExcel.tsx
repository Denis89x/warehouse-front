...
export const ProductChartExcel: React.FC<ProductProps> = ({startDate, endDate}) => {
    const handleExcel = async (startDate: string, endDate: string) => {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/api/v1/products/excel-chart?startDate=${startDate}&endDate=${endDate}`, {
                headers,
                responseType: 'blob',
            });
            ...
            if (response.status === 200) {
                ...
                link.href = url;
                link.setAttribute('download', 'ProductChart.xlsx');
