...
const FetchProductType = () => {
    const {productTypes, updateProductTypes, searchProductType} = useProductTypeContext();
    ...
    const handleDelete = async (productTypeId: number) => {
        try {
            ...
            const response = await axios.delete(`http://localhost:8080/api/v1/types/${productTypeId}`, {
                headers,
            });
                        ...
                        <tr key={index}>
                            <td>
                                <div className={"crud-actions"}>
                                    {type.name}
                                    <div>
                                        <Link to={`/edit-type/${type.productTypeId}`}>
                                        ...
                                        <button onClick={() => handleDelete(type.productTypeId)} type={"button"}
                                                className={"button"}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
                                                 viewBox="0 0 24 24">
                                                <g fill="none">
...
export default FetchProductType;
